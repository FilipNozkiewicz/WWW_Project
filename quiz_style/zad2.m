...
[y,fpr2,Nbits2]=wavread('coldvox.wav');      % wczytaj sygnaďż˝ mowy (caďż˝y)
...
for  nr = 1 : Nramek
    ...
    if ( rmax > 0.35*r(1) ) T=imax; else T=0; end % gďż˝oska dďż˝wiďż˝czna/bezdďż˝wiďż˝czna?
    % if (T>80) T=round(T/2); end							% znaleziono drugďż˝ podharmonicznďż˝
    %T=2*T;	   %podpunkt 2.			% wyďż˝wietl wartoďż˝ďż˝ T
    rr(1:Np,1)=(r(2:Np+1))';
    for m=1:Np
        R(m,1:Np)=[r(m:-1:2) r(1:Np-(m-1))];			% zbuduj macierz autokorelacji
    end
    ...
    H=freqz(1,[1;a]);
    by=filter([1;a],1,bx);
by=by*1000;										% oblicz jego odp. czďż˝stotliwoďż˝ciowďż˝
    ...
    % SYNTEZA - odtwďż˝rz na podstawie parametrďż˝w ----------------------------------------------------------------------
     T = 0;    %podpunkt 1                                    % usuďż˝ pierwszy znak ďż˝%ďż˝ i ustaw: T = 80, 50, 30, 0 (w celach testowych)
	...
	pob=10*(y(nr*Mstep+n)); gdzie=(3/2)*Mstep+1;%podpunkt 4	
        else
	%T=70; podpunkt 3
            if (n==gdzie)
	 pob=1; gdzie=gdzie+T;	   % pobudzenie dďż˝wiďż˝czne
	%pob=by(n); gdzie=gdzie+T;%pobudzenie sygnalem resztkowym
            else pob=0; end
        end
